...
def test_hartree_fock_hessian(method='hf'):
    ...
    n = psi_deriv.shape[0]
    quax_deriv = np.asarray(quax.core.derivative("geom.xyz", basis_name, method, deriv_order=2, options=options)).reshape(n,n)
    quax_partial00 = quax.core.partial_derivative("geom.xyz", basis_name, method, deriv_order=2, partial=(0,0))
